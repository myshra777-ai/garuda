...
Observation — AST structures, resolved symbols, source declarations, runtime telemetry, runtime spans.

Claim — calls, interface, dependency relationships, repository topology.

Evidence — source files, line locations, revisions, runtime observations, provenance.

Verification — SUPPORTED, UNVERIFIED, or CONTRADICTED.

Decision — architectural choices, accepted / rejected approaches, policies, unresolved decisions.

Lineage — how the current state evolved.
...
For full methodology, screenshots, validation artifacts, and historical runs, see EVIDENCE.md.
...
A complete UI walkthrough is available in docs/WALKTHROUGH.md.
...
For detailed installation, operational procedures, and workflow examples, see PLAYBOOK.md.
...
Unknown stays unknown — Missing evidence must not silently become certainty.

Identity is deterministic — The same semantic entity should remain identifiable across analysis runs.

State is auditable — Important state transitions should leave an inspectable trail.

Architecture should be navigable — Users should move from system-level topology to specific entities and evidence.

Static and runtime knowledge remain distinct — Complementary signals, not interchangeable facts.

AI should consume structured state — Agents should not reconstruct an entire repository from raw text whenever reusable semantic state already exists.

Correctness before scale — Validated semantics and evidence before expanding language and deployment scope.
...
Evidence	Validation results, benchmarks, screenshots and methodology
Benchmarks (v1.0.0)	System verification & benchmark report
Architecture	Technical architecture and design
Walkthrough	Dashboard and IDE workflow tour
...
Reproducibility — For reproducible validation, run the included benchmark and review the evidence artifacts in EVIDENCE.md and BENCHMARKS_AND_VERIFICATION_v1.0.0.md.

<p align="center"> <strong>Garuda</strong><br> Analyze | Verify | Govern | Collaborate </p><p align="center"> <sub> Built as a shared intelligence workspace.<br> If the system can provide evidence, don't replace it with a guess.<br> If the system can verify, don't rely on assumption alone.<br> If the context already exists, don't make every human or AI agent rediscover it. </sub> </p><p align="center"> <sub> <strong>Version:</strong> v1.0.0 · <strong>License:</strong> Apache 2.0 </sub> </p> ```

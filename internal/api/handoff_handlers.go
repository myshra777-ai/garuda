...
func (s *Server) HandleHandoff(w http.ResponseWriter, r *http.Request) {
	...
	ps, ok := s.store.(interface {
		ExecuteHandoffTransaction(context.Context, *store.HandoffRequest) (*store.HandoffResponse, error)
	})
	if !ok {
	...
	if s.sseBroker != nil {
		s.sseBroker.Publish(telemetry.EventType("agent_handoff"), map[string]interface{}{
			"handoff_id": resp.HandoffID, "checkpoint_id": resp.CheckpointID, "task_id": resp.TaskID,
			"source_agent_id": req.SourceAgentID, "target_agent_id": req.TargetAgentID,
			"reason": req.Reason, "status": resp.Status, "request_id": requestID,
		})
	}
...
func (s *Server) HandleResume(w http.ResponseWriter, r *http.Request) {
	...
	rs, ok := s.store.(interface {
		ResumeAgent(context.Context, uuid.UUID, uuid.UUID, uuid.UUID) (interface{}, error)
	})
	if !ok {
	...
	if err != nil {
		slog.Error("resume agent failed", "error", err, "request_id", requestID, "agent_id", req.AgentID, "checkpoint_id", req.CheckpointID)
	...
	if s.sseBroker != nil {
		s.sseBroker.Publish(telemetry.EventType("agent_resumed"), map[string]interface{}{
			"agent_id": req.AgentID, "checkpoint_id": req.CheckpointID, "status": "working", "request_id": requestID,
		})
	}
	w.Header().Set("Content-Type", "application/json")
	w.WriteHeader(http.StatusOK)
	_ = json.NewEncoder(w).Encode(map[string]interface{}{
		"agent_id": req.AgentID, "checkpoint_id": req.CheckpointID, "status": "working", "state": restoredState, "request_id": requestID,
	})
}
...
func (s *Server) HandleGetLineage(w http.ResponseWriter, r *http.Request) {
	...
	ls, ok := s.store.(interface {
		GetLineageDAG(context.Context, uuid.UUID, uuid.UUID) ([]types.LineageEdge, error)
	})
	if !ok {
	...
	if err != nil {
		slog.Error("failed to query lineage DAG", "error", err, "request_id", requestID, "task_id", taskID, "tenant_id", tenantID)
	...
	w.WriteHeader(http.StatusOK)
	_ = json.NewEncoder(w).Encode(map[string]interface{}{
		"task_id": taskID, "edges": edges, "total": len(edges), "request_id": requestID,
	})
}

...
**Generated:** Fri Aug 14 12:06:33 AM IST 2026
...
--------------------+--------------------------+-----------+----------+---------------
 ...
 signature          | bytea                    |           |          | 
 merkle_hash        | text                     |           |          | 
 parent_merkle_hash | text                     |           |          | 
 scope_domain       | text                     |           |          | ''::text
 scope_system       | text                     |           |          | ''::text
...
Indexes:
    ...
    "idx_decisions_domain" btree (domain)
    "idx_decisions_merkle_hash" btree (merkle_hash)
    "idx_decisions_parent" btree (parent_id)
    "idx_decisions_repo" btree (repository_id)
    "idx_decisions_scope" gin (scope)
    "idx_decisions_system" btree (system)
    "idx_decisions_tenant_created" btree (tenant_id, created_at DESC)
    "idx_decisions_tenant_status" btree (tenant_id, status)
    "idx_decisions_valid_range" btree (tenant_id, valid_from, valid_to)
    "idx_decisions_workspace" btree (workspace_id)
Check constraints:
    "check_valid_range" CHECK (valid_to IS NULL OR valid_from <= valid_to)
Referenced by:
    TABLE "contradictions" CONSTRAINT "fk_contradictions_decision_a" FOREIGN KEY (tenant_id, decision_a) REFERENCES decisions(tenant_id, id) ON DELETE CASCADE
    TABLE "contradictions" CONSTRAINT "fk_contradictions_decision_b" FOREIGN KEY (tenant_id, decision_b) REFERENCES decisions(tenant_id, id) ON DELETE CASCADE
...
------------------------+--------------------------+-----------+----------+----------------------------------------------
 ...
 decision_hash          | bytea                    |           |          | 
 previous_revision_hash | bytea                    |           |          | 
 canonical_json         | jsonb                    |           |          | 
Indexes:
    "decision_revisions_pkey" PRIMARY KEY, btree (id)
    "idx_decision_revisions_decision" btree (decision_id)
    "idx_decision_revisions_decision_revision" UNIQUE, btree (decision_id, revision_number)
    "idx_revisions_created" btree (tenant_id, created_at DESC)
    "idx_revisions_decision" btree (tenant_id, decision_id, revision_number DESC)
...
------------+--------------------------+-----------+----------+----------------------------------------------
 ...
 created_at | timestamp with time zone |           | not null | now()
 tenant_id  | uuid                     |           | not null | '00000000-0000-0000-0000-000000000001'::uuid
Indexes:
    "evidence_store_pkey" PRIMARY KEY, btree (tenant_id, block_hash)
    "idx_evidence_created" btree (created_at)
...
--------------+--------------------------+-----------+----------+---------
 ...
 block_height | bigint                   |           | not null | 0
 created_at   | timestamp with time zone |           | not null | now()
 updated_at   | timestamp with time zone |           | not null | now()
Indexes:
    "merkle_roots_pkey" PRIMARY KEY, btree (tenant_id)
    "idx_merkle_roots_tenant" btree (tenant_id)
...
Foreign-key constraints:
    "fk_contradictions_decision_a" FOREIGN KEY (tenant_id, decision_a) REFERENCES decisions(tenant_id, id) ON DELETE CASCADE
    "fk_contradictions_decision_b" FOREIGN KEY (tenant_id, decision_b) REFERENCES decisions(tenant_id, id) ON DELETE CASCADE
...
### Workspace Store Methods
  ...
  - func (s *PostgresStore) AddRepository(ctx context.Context, tenantIDStr, workspaceName, provider, url, defaultBranch, language string) (*Repository, error) {
...
---
✅ Truth Index generated: garuda_truth_index_20260814_000633.md
